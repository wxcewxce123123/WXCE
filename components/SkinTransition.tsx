...
const SkinTransition: React.FC<SkinTransitionProps> = ({ targetSkin, onPeak, onComplete }) => {
  ...
  useEffect(() => {
    ...
    let frame = 0;
    const TOTAL_FRAMES = 150; // Increased from 90 to 150 for smoother transition
    const PEAK_FRAME = 75;    // Middle point
    let hasPeaked = false;
    ...
    // --- Particle Classes (Slowed Down) ---
    class Leaf {
        x: number; y: number; size: number; color: string; vx: number; vy: number; rot: number;
        constructor() { this.x = -50; this.y = Math.random() * height; this.size = Math.random() * 20 + 10; this.color = Math.random() > 0.5 ? '#10b981' : '#065f46'; this.vx = Math.random() * 8 + 6; this.vy = (Math.random() - 0.5) * 2; this.rot = Math.random() * Math.PI; }
    ...
    class Bubble { x: number; y: number; r: number; speed: number; constructor() { this.x = Math.random() * width; this.y = height + Math.random() * 100; this.r = Math.random() * 50 + 20; this.speed = Math.random() * 6 + 4; } update() { this.y -= this.speed; } draw(ctx: CanvasRenderingContext2D) { ctx.beginPath(); ctx.arc(this.x, this.y, this.r, 0, Math.PI*2); ctx.fillStyle = '#0ea5e9'; ctx.fill(); } }
    class SakuraPetal { x: number; y: number; size: number; color: string; vx: number; vy: number; rot: number; constructor() { this.x = Math.random() * width + width + 50; this.y = Math.random() * height * 1.5 - height * 0.25; this.size = Math.random() * 20 + 10; this.color = Math.random() > 0.5 ? '#fda4af' : '#fecdd3'; this.vx = -(Math.random() * 10 + 5); this.vy = Math.random() * 2 - 1; this.rot = Math.random() * Math.PI; } update() { this.x += this.vx; this.y += this.vy; this.rot += 0.05; } draw(ctx: CanvasRenderingContext2D) { ctx.save(); ctx.translate(this.x, this.y); ctx.rotate(this.rot); ctx.fillStyle = this.color; ctx.beginPath(); ctx.moveTo(0,0); ctx.quadraticCurveTo(this.size/2, -this.size, this.size, 0); ctx.quadraticCurveTo(this.size/2, this.size, 0,0); ctx.fill(); ctx.restore(); } }
    class SandGrain { x: number; y: number; w: number; speed: number; constructor() { this.x = width + Math.random() * 500; this.y = Math.random() * height; this.w = Math.random() * 60 + 10; this.speed = Math.random() * 15 + 10; } update() { this.x -= this.speed; } draw(ctx: CanvasRenderingContext2D) { ctx.fillStyle = '#fde047'; ctx.globalAlpha = 0.6 * globalFade; ctx.fillRect(this.x, this.y, this.w, 3); ctx.globalAlpha = globalFade; } }
    class WarpStar { x: number; y: number; z: number; constructor() { this.x = (Math.random() - 0.5) * width; this.y = (Math.random() - 0.5) * height; this.z = Math.random() * width; } update() { this.z -= 15; if(this.z <= 1) { this.z = width; this.x=(Math.random()-0.5)*width; this.y=(Math.random()-0.5)*height; } } }
    class MatrixChar { x: number; y: number; speed: number; char: string; constructor() { this.x = Math.floor(Math.random() * (width/15)) * 15; this.y = Math.random() * -height; this.speed = Math.random() * 10 + 5; this.char = Math.random() > 0.5 ? '1' : '0'; } update() { this.y += this.speed; } draw(ctx: CanvasRenderingContext2D) { ctx.fillStyle = '#0f0'; ctx.font = '20px monospace'; ctx.fillText(this.char, this.x, this.y); } }
    class InkBlob { x: number; y: number; r: number; maxR: number; speed: number; constructor() { this.x = Math.random() * width; this.y = Math.random() * height; this.r = 0; this.maxR = Math.max(width, height) * 0.6; this.speed = Math.random() * 6 + 3; } update() { if(this.r < this.maxR) this.r += this.speed; } draw(ctx: CanvasRenderingContext2D) { ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(this.x, this.y, this.r, 0, Math.PI*2); ctx.fill(); } }
    class FrostSpike { x: number; y: number; h: number; width: number; constructor() { this.x = Math.random() * width; this.y = Math.random() > 0.5 ? 0 : height; this.h = 0; this.width = Math.random() * 30 + 10; } update() { this.h += 10; } draw(ctx: CanvasRenderingContext2D) { ctx.fillStyle = '#e0f2fe'; ctx.beginPath(); if (this.y === 0) { ctx.moveTo(this.x, 0); ctx.lineTo(this.x - this.width/2, this.h); ctx.lineTo(this.x + this.width/2, 0); } else { ctx.moveTo(this.x, height); ctx.lineTo(this.x - this.width/2, height - this.h); ctx.lineTo(this.x + this.width/2, height); } ctx.fill(); } }

    class Gear {
      x: number; y: number; r: number; teeth: number; rot: number; speed: number;
      constructor() { this.x = Math.random()*width; this.y = Math.random()*height; this.r=0; this.teeth=8; this.rot=0; this.speed=0.04; }
      update() { this.r += 4; this.rot += this.speed; }
    ...
    class DivineRay {
      ...
      constructor() {
        ...
        this.speed = (Math.random() - 0.5) * 0.01; // Slower rotation
    ...
    class BurstParticle {
        ...
        constructor(type: 'feather' | 'spark') {
           ...
           // Explosive speed slowed down
           const speed = type === 'feather' ? (Math.random() * 8 + 2) : (Math.random() * 12 + 4);
           this.vx = Math.cos(angle) * speed;
           this.vy = Math.sin(angle) * speed;
           this.size = type === 'feather' ? (Math.random() * 15 + 5) : (Math.random() * 4 + 1);
           this.color = type === 'feather' ? '#fff' : '#fbbf24';
           this.decay = Math.random() * 0.98 + 0.96; // Less drag, smoother float
        }
    ...
    const animate = () => {
        ...
        else if (targetSkin === 'forest') { const alpha = progress < 0.5 ? progress * 2 : 2; ctx.fillStyle = `rgba(20, 83, 45, ${alpha > 1 ? 1 : alpha})`; ctx.fillRect(0, 0, width, height); particles.forEach(p => { p.update(); p.draw(ctx); }); if (frame < 60 && frame % 4 === 0) particles.push(new Leaf()); }
